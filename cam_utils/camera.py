...
class Camera:
    ...
    def resize_v2(self, factor_x, factor_y):
        self.K[..., 0, 0] *= factor_x
        self.K[..., 1, 1] *= factor_y
        self.K[..., 0, 2] *= factor_x
        self.K[..., 1, 2] *= factor_y
        self.params[..., 0] *= factor_x
        self.params[..., 1] *= factor_y
        self.params[..., 2] *= factor_x
        self.params[..., 3] *= factor_y
        return self
    ...
    def flip(self, H, W, direction="horizontal"):
        # new_fx = -self.params[:, 0] if direction == "horizontal" else self.params[:, 0]
        # new_fy = -self.params[:, 1] if direction == "vertical" else self.params[:, 1]
        new_fx = self.params[:, 0]
        new_fy = self.params[:, 1]
        new_cx = (
        ...
        self.params[:, :4] = torch.stack(
            [new_fx, new_fy, new_cx, new_cy], dim=1
        )
        self.K[..., 0, 0] = new_fx
        self.K[..., 1, 1] = new_fy
        self.K[..., 0, 2] = new_cx
...
class EUCM(Camera):
    ...
    def project(self, xyz):
        H, W = xyz.shape[-2:]
        fx, fy, cx, cy, alpha, beta = self.params[..., :6].unbind(dim=-1)       # [b]
        fx, fy = fx[..., None, None], fy[..., None, None]
        cx, cy = cx[..., None, None], cy[..., None, None]
        alpha, beta = alpha[..., None, None], beta[..., None, None]
        x, y, z = xyz.unbind(dim=1)                                             # [b, h, w]
        d = torch.sqrt(beta * (x**2 + y**2) + z**2)                             # [b, h, w]

        x = x / (alpha * d + (1 - alpha) * z).clip(min=1e-3)                   # [b, h, w]
        y = y / (alpha * d + (1 - alpha) * z).clip(min=1e-3)                   # [b, h, w]

        Xnorm = fx * x + cx         # [b, h, w]
        Ynorm = fy * y + cy         # [b, h, w]

        coords = torch.stack([Xnorm, Ynorm], dim=1)     # [b, 2, h, w]
...
#         eps = 1e-6
#         B, _, H, W = xyz.shape
#         fx, fy, cx, cy, xi, alpha = self.params.unbind(dim=1)

#         x, y, z = xyz.unbind(dim=1)
#         r2 = x * x + y * y + z * z
#         d1 = torch.sqrt(r2).clamp_min(eps)                        # ||X||
#         z_xi = xi.view(B, 1, 1) * d1 + z                          # xi * ||X|| + z
#         d2 = torch.sqrt(x * x + y * y + z_xi * z_xi).clamp_min(eps)

#         denom = (alpha.view(B, 1, 1) * d2 + (1.0 - alpha.view(B, 1, 1)) * z_xi).clamp_min(eps)

#         u = fx.view(B, 1, 1) * (x / denom) + cx.view(B, 1, 1)
#         v = fy.view(B, 1, 1) * (y / denom) + cy.view(B, 1, 1)

#         coords = torch.stack([u, v], dim=1)

#         # Validity: denominator positive AND point not behind the front sphere (z_xi > 0)
#         invalid = (
#             (coords[:, 0] < 0)
#             | (coords[:, 0] > W)
#             | (coords[:, 1] < 0)
...
#     @torch.autocast(device_type="cuda", enabled=False, dtype=torch.float32)
#     def unproject(self, uv):
#         # uv: [B, 2, H, W] with pixel coordinates
#         eps = 1e-6
#         B, _, H, W = uv.shape
#         u, v = uv.unbind(dim=1)
#         fx, fy, cx, cy, xi, alpha = self.params.unbind(dim=1)

#         mx = (u - cx.view(B, 1, 1)) / fx.view(B, 1, 1)
#         my = (v - cy.view(B, 1, 1)) / fy.view(B, 1, 1)
#         r2 = mx * mx + my * my

#         # Domain validity for the square-root term:
#         # sqrt(1 - (2*alpha - 1) * r2) must be real
#         two_a_minus_1 = (2.0 * alpha - 1.0).view(B, 1, 1)
#         sqrt_arg = 1.0 - two_a_minus_1 * r2
#         valid_mask = sqrt_arg > 0

#         # m_z as in Usenko et al. (Double Sphere)
#         a = alpha.view(B, 1, 1)
#         mz_num = 1.0 - (a * a) * r2
#         mz_den = a * torch.sqrt(sqrt_arg.clamp_min(eps)) + (1.0 - a)
#         mz = mz_num / mz_den.clamp_min(eps)

#         # k factor
#         xi_b = xi.view(B, 1, 1)
#         k_sq = mz * mz + (1.0 - xi_b * xi_b) * r2
#         k = (mz * xi_b + torch.sqrt(k_sq.clamp_min(eps))) / (r2 + mz * mz).clamp_min(eps)
...
#         ray = ray / torch.norm(ray, dim=1, keepdim=True).clamp_min(1e-4)

#         self.unprojection_mask = valid_mask & (z > 0)
#         return ray
...
class Spherical(Camera):
    ...
    def resize_v2(self, factor_x, factor_y):
        ...
        self.params[..., 2] *= factor_x
        self.params[..., 3] *= factor_y
        self.params[..., 4] *= factor_x
        self.params[..., 5] *= factor_y
        return self
    ...
    # debug
    def rotate(self, image, R, mode="bilinear"):
        # [b, 2, h, w]
        id_coords = coords_grid(1, image.shape[-2], image.shape[-1], device=image.device)
        b, _, h, w = id_coords.shape
        # [b, 3, h, w]
        xyz = self.unproject(id_coords)
        xyz = R[None] @ xyz.view(b, 3, -1)      # [b, 3, hw]
        xyz = xyz.view(b, 3, h, w)
        id_coords = self.project(xyz)           # [b, 2, h, w]

        # norm to [-1, 1] for grid sample
        norm_id_coords = id_coords.clone()
        norm_id_coords[:, 0] = norm_id_coords[:, 0] / (w - 1) * 2 - 1
        norm_id_coords[:, 1] = norm_id_coords[:, 1] / (h - 1) * 2 - 1
        ...
        if image.dtype == torch.uint8:
            ...
            rotated_image = F.grid_sample(image, norm_id_coords.permute(0, 2, 3, 1), mode=mode, align_corners=True)
            rotated_image = (rotated_image * 255.).to(torch.uint8)
        elif image.dtype == torch.bool:
            rotated_image = F.grid_sample(image.float(), norm_id_coords.permute(0, 2, 3, 1), mode=mode, align_corners=True)
            rotated_image = rotated_image > 0.5
        else:
            rotated_image = F.grid_sample(image, norm_id_coords.permute(0, 2, 3, 1), mode=mode, align_corners=True)
...
class BatchCamera(Camera):
    ...
    def resize_v2(self, ratio_x, ratio_y):
        val = torch.cat([camera.resize_v2(ratio_x, ratio_y) for i, camera in enumerate(self.cameras)])
        return val
    ...
    def __setitem__(self, idx, value):
        ...
        if isinstance(idx, int):
            ...
            if self.params.dim() == 2:
                ...
                self.params[idx, :, : value.params.shape[1]] = value.params
            else:
                raise NotImplementedError(f"Invalid params shape: {self.params.shape}")

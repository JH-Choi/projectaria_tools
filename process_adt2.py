# https://github.com/facebookresearch/egolifter/blob/d177089a153831e4cd592fb09c8f86f8a63d398c/scripts/process_adt_3dgs.py#L302
...
class ProcessAriaDigitalTwin:
    data_root: Path = Path("./temp") 
    sequence_name: str = "Apartment_release_golden_skeleton_seq100"
    output_root: Path = Path("./temp2")
    
    output_rgb_width: int = 1408
    output_rgb_height: int = 1408
    output_rgb_focal: int = 460
    
    save_rectified_images: bool = False
    ...
    def main(self) -> None:
        ...
        for selected_device_number in range(len(subseq_paths)):
            ...
            for idx, timestamp_ns in enumerate(tqdm(img_timestamps_ns)):
                ...
                depth_with_dt = gt_provider.get_depth_image_by_timestamp_ns(timestamp_ns, stream_id)
                # if abs(depth_with_dt.dt_ns()) > 3_000_000: # 3 ms
                #     print('skip depth image 1')
                #     continue
                # # assert depth_with_dt.is_valid(), "depth not valid for input timestamp!"
                if not depth_with_dt.is_valid():
                    print('skip depth image 2')
                    continue
                # assert depth_with_dt.is_valid(), "Depth not valid for input timestamp!"
                ...
                if self.save_rectified_images:
                    ...
                    image_raw_output_subpath = f"images_rectified/rgb_{timestamp_ns}.jpg"
                ...
                depth_output_subpath = f"depths/depth_{timestamp_ns}.npy"
                depth_image = depth_with_dt.data().to_numpy_array()
                depth_image_output_path = output_folder / depth_output_subpath
                os.makedirs(os.path.dirname(depth_image_output_path), exist_ok=True)
                np.save(depth_image_output_path, depth_image)


                # Save the depth visualization image
                depth_viz_output_subpath = f"depths_viz/depth_{timestamp_ns}.jpg"
                depth_viz_output_path = output_folder / depth_viz_output_subpath
                os.makedirs(os.path.dirname(depth_viz_output_path), exist_ok=True)
                ...
                imageio.imwrite(depth_viz_output_path, depth_for_vis)
